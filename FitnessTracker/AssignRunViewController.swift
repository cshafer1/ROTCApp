...
class AssignRunViewController: UIViewController {
    ...
    func assignRunCallback(obj: PFObject?, err:Error?){
        ...
        temp!["RunData"] = [Float(lengthField.text as! String), timeField.text]
        temp?.saveInBackground();
        self.dismiss(animated: true, completion: nil)
    }
    
    @IBAction func assignRun(_sender: UIButton) {
        if (group == nil || lengthField.text == nil || timeField.text == nil || user == nil) {
        ...
        db.queryFirstObject(predicateStr: "netid == '"+name+"'", className: "Runs", callback: assignRunCallback(obj:err:))
